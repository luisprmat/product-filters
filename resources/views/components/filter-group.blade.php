...
<div x-data="{
    ...
    <h3 class="mt-2 mb-1 text-xl">
        <button @click="open = !open" class="flex w-full items-center justify-between text-left">
            <div>{{ $title }} (<span x-text="options.length"></span>)</div>
    ...
    >
        <input x-model="search" class="mb-2 w-full rounded-md border border-gray-400 px-2 py-1 text-sm" placeholder="Buscar {{ strtolower($title) }}">
        ...
        <template x-if="search.length === 0">
            <div>
                ...
                <template x-if="endIndex === options.length">
                    <div class="mt-1 cursor-pointer font-medium text-indigo-500" @click="endIndex = {{ $endIndex }}">Mostrar menos ...</div>
                </template>
            </div>
        </template>
    </div>
